...
export const traverseImports = async (data, baseURL) => {
    ...
    if (imports) {
        for (const importPath of imports) {
            const path = importPath.match(/require\("(.+)"\)/)[1];
...
export const installAddon = async (baseURL) => {
    ...
    fs.writeFile(path.join(dir, "velocity_manifest.json"), JSON.stringify(data.manifest));
    fs.writeFile(path.join(dir, data.manifest.main), data.main);
    ...
    imports.forEach(async (importData) => {
        try {
            fs.writeFile(path.join(dir, importData.path), await VelocityCore.request(importData.url));
        } catch (e) {
